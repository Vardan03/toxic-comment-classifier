...
class BERTModel(nn.Module):
    ...
    Changes:
    1. unfreeze_last_n (default=2) — last N transformer blocks unfrozen
    2. Discriminative learning rates — head: lr, BERT layers: lr * 0.1
    3. batch_size=16 + accumulation_steps=2 — effective batch=32, VRAM efficient
    4. pooler_output instead of last_hidden_state[:, 0, :]
    5. pos_weight clip(max=100) instead of max=40
    6. Threshold search expanded to [0.1, 0.90] step 0.02
    7. recall added to evaluate() and epoch print
    8. threshold defaults to 0.5 in predict() and evaluate()
    9. _find_best_threshold optimizes F1 with recall >= 0.85 constraint
    """
    ...
    def _get_bert_unfrozen_params(self) -> List[nn.Parameter]:
        ...
        print(
            f"[BERTModel] unfrozen blocks: {self.unfreeze_last_n}/{n_layers} "
            f"(layers {n_layers - self.unfreeze_last_n}–{n_layers - 1})"
        )
        return unfrozen
    ...
    def fit(
        ...
        optimizer = torch.optim.AdamW(
            [
                ...
                {"params": bert_params,            "lr": self.lr * self.bert_lr_multiplier},
            ],
            weight_decay=0.01,
        )
